...
class RegionalControl(Document):
    ...
    created = IntField(required=True, default=lambda: int(datetime.utcnow().timestamp()), verbose_name="创建时间")
    update_time = IntField(required=True, default=lambda: int(datetime.utcnow().timestamp()), verbose_name="修改时间")
