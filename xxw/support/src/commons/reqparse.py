...
from src.commons.error_handler import RequestsError
...
class Argument(object):
    ...
    :param desc: name description, for error output.
    :param int code: error code, for error output.
    """
    ...
    def customize_source(self, request):
        """
        限定GET方法使用request.values, POST,PATCH,DELETE等方法使用request.json
        """
        values = MultiDict()
        method = "values" if request.method == "GET" else "json"
    ...
    def convert(self, value, op):
        ...
        if value is None:
            if self.nullable:
                ...
                raise RequestsError(code=self.code)
        ...
        try:
            # 接口参数为list的情况, 例如['value1', 'value2']
            if self.type.__name__ == "list":
                return value
    ...
    def parse(self, request, bundle_errors=False):
        ...
        :param do not abort when first error occurs, return a
            ...
            bundled
        """
        source = self.customize_source(request)
